...
class CreateListingForm(forms.ModelForm):
    ...
    description = forms.CharField(label="Description", widget=forms.Textarea(attrs={
                                    'placeholder': "Describe the digital art",
    ...
    close_date = forms.DateTimeField(label='Close Date (Dd-Mm-Yy Hh:Mm)',
                                          label_suffix=" : ", required=True, disabled=False,
                                          input_formats=["%d-%m-%Y %H:%M"],
                                          widget=forms.DateTimeInput(attrs={'class': 'form-control'}),
                                          error_messages={'required': "This field is required."})
    current_price = forms.DecimalField(label='ETH Value', label_suffix=" : ", widget=forms.NumberInput(attrs={
                                        "class": "form-control"})

    )
                                

    #Can we add close date to be date format? Changed but now not saving....need to filter input so it gets "%d-%m-%Y %H:%M" format.

    class Meta:
        model = Auction
        fields = ["title", "description", "category", "image_url", "close_date"]
...
# -- add a page for partners  -------------------------------------------------- NEED TO ASSESS THE auctions part!!! 

def partners(request):
    return render(request, "auctions/partners.html")

# -- add a page for merch  --- 

def merch(request):
    return render(request, "auctions/merch.html")

# -- add a page for marketplace  --- 

def footprint(request):
    return render(request, "auctions/footprint.html")

def about_us(request):
    return render(request, "auctions/about_us.html")

def purpose(request):
    return render(request, "auctions/purpose.html")
